...
\textit{Theorem 9.1*} {\kaishu 广义特征空间分解的实向量空间版本}
...
\textit{Theorem 9.2* Jordan}{\kaishu 基存在性的实向量空间版本}
...
因此若存在\(p<m_1+1\)使得\((T-a_iI)^p u_1^0=0\)或\((T-a_iI)^p v_1^0=0\)，那么
    ...
    \end{align*}
从而导致递推链出现冗余，因此\(u,v,(u,v)\)的幂零指数严格相等.将上述递推链变形为
    \begin{align*}
        \forall k=0,\cdots,m_i,Tu_1^k=u_1^{k+1}+a_i u_1^k-b_i v_1^k,Tv_1^k=v_1^{k+1}+a_i v_1^k+b_i u_1^k
    \end{align*}
这组基下的\(M(T|_{U_i},(u_1^{m_1},v_1^{m_1},\cdots,u_1^0,v_1^0))\)是实向量空间的\textit{Jordan}标准型.
    \begin{align*}
        M(T_i)=
        \begin{pmatrix}
            C      & I       & \cdots & 0      \\
        ...
        \end{pmatrix}
    \end{align*}
于是\(M(T)=\mathrm{diag}(M(T_1),\cdots,M(T_q))\)，其中\(U_1,\cdots,U_q\)是\(G(\lambda_i,T_C)\)对应的所有循环子空间.
