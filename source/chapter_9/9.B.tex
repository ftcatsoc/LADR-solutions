\textit{Theorem 9.34} {\kaishu 实内积空间上的正规算子}
...
由于实向量空间上的算子一定有一维或二维不变子空间，故设\(e_1,e_2\)满足
    ...
    \end{align*}
令\(U=\altspan(e_1,e_2)\)，考虑\(U^\bot\).根据\textit{7.B.20*}，\(T|_{U^\bot}\)是正规算子，
...
\textit{Theorem 9.35} {\kaishu 实内积空间上的等距算子}
设\(V\)是有限维实内积空间且\(T \in L(V)\).
...
或\(\forall j=1,\cdots,q,\norm{Se_{2j-1}}^2=\norm{a_je_{2j-1}-b_je_{2j}}^2=a_j^2+b_j^2=1=\norm{e_j}^2\).

于是有\(\abs{t_j}=1\)且\(a_j^2+b_j^2=1\).使用三角换元，存在\(\theta_j\)使得\(a_j=\cos \theta_j,b_j=\sin \theta_j\).
