...
(\textit{c})证明：\(V\)中存在某个向量\(v\)和某个满足\(\dim U \leq m-1\)的子空间\(U\)，使得\(A=v+U\).
...
\textit{Proof}
先证\(V=\altspan (v_1,\cdots,v_m,u_1,\cdots,u_n)\).

\(\forall v+U \in V/U, \exists a_i \in F\)，使得\(v+U=\sum_{i=1}^m a_i(v_i+U)=\sum_{i=1}^m a_iv_i+U\).

从而\(v-\sum_{i=1}^m a_iv_i \in U\)，即\(\exists u \in \altspan (u_1,\cdots,u_n) \in U,v-\sum_{i=1}^m a_iv_i=u\).

最终\(v=\sum_{i=1}^m a_iv_i+u \in \altspan (v_1,\cdots,v_m,u_1,\cdots,u_n)\)，证毕.

再证\(v_1,\cdots,v_m,u_1,\cdots,u_n\)线性无关.由于\(v_1+U,\cdots,v_m+U\)是\(V/U\)的一组基，

故\(\sum_{i=1}^m a_i(v_i+U)=U \Rightarrow a_1=\cdots=a_m=0\).

因此\(v_1,\cdots,v_m\)线性无关且\(\altspan (v_1,\cdots,v_m) \cap U=\{0\}\).

根据定理2.34和\textit{2.B.8}，\(V=\altspan (v_1,\cdots,v_m) \oplus U\)，
