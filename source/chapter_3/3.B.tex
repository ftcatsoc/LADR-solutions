...
(\textit{a})若\(V=\altspan (v_1,\cdots,v_m)\)，则\(T\)具有怎样的性质？
...
\textit{17.}
设\(V\)和\(W\)是有限维的.求证：当且仅当\(\dim V\leq \dim W\)，存在单射的\(T\in L(V,W)\).

\textit{Proof}
必要性：若存在单射的\(T\in L(V,W)\)，

则\(\dim V=\dim \Ker T+\dim \Img T=\dim \Img T \leq \dim W\)，证毕.
...
\textit{Proof}
必要性：原式等价于\(\dim U=\dim \Ker T=\dim V-\dim \Img T \geq \dim V-\dim W\)，

即\(\dim \Img T \leq \dim W\)，证毕.
...
\(\Ker T=U\)显然成立，同时\(\Img T=\altspan (Tv_{m+1},\cdots,Tv_{m+n})\).

由于\(\dim \Img T\leq \dim W\)，这要求\(n\leq \dim W\)，与条件相符.
...
\begin{comment}
    \textit{26.}
    设\(D \in L(P(R))\)对于其中任意的多项式\(p\)满足\(\altdeg Dp=\altdeg p-1\).
    ...
    根据\textit{3.B.10}，命题等价于
        ...
        \end{align*}
    根据\textit{2.C.10}，由于\(\altdeg Dp=\altdeg p-1\)，
